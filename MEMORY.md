...
- **Facebook Leads Driver**: Integrated `FacebookLeadsDriver` under the `facebook_leads` channel, synchronizing lead entities dynamically using page form leads, storing the results in `ChanneledCustomer` entities, and using synthetic emails (`{leadId}@leads.facebook.com`) when raw emails are not captured by forms to align with the nullable email constraint.
